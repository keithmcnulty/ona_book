...
We can see that there are two types of nodes: `Person` and `Movie`.  We can also see that a `Person` can follow another `Person`, while there are five relationship types between a `Person` and a `Movie`.  Cypher uses `r if (knitr::is_latex_output()) '\\index{ASCII art}'`ASCII art to make queries easier to understand and write.  Nodes are written in parentheses such as `(p:Person)` and relationships are written in lines or arrows such as `-[:ACTED_IN]->`.  The following Cypher query will return the graph of all people who acted in movies in the database which were directed by Clint Eastwood, with the results displayed in Figure \@ref(fig:clinteastwood).
...
Each node or relationship may contain `r if (knitr::is_latex_output()) '\\index{edge!properties}'``r if (knitr::is_latex_output()) '\\index{vertex!properties}'`properties which can be accessed and returned in queries.  For example, `Movie` nodes have `title` and `released` properties, while `Person` nodes have a `name` property.  This query will return the actor name, movie title and movie release data, and the result looks like Figure \@ref(fig:clint-results).
...
This is the core of the query---the remainder simply counts the occurrences of the different country labels and then ranks them in descending order and returns the top 10.  You can see how this query requires quite detailed instruction on how to traverse the graph and also needs a very detailed knowledge of a bewildering array of codes for properties and values.  RDFs are beautiful and extremely powerful relationship storage engines, but there is a high expertise bar to their use.
...
In the following examples, we will load our `r if (knitr::is_latex_output()) '\\index{data sets!ontariopol@\\texttt{ontariopol}}'``ontariopol` data set of the Ontario state politician Twitter network from Chapter \@ref(community) into a Neo4J graph database and do some querying against the database.
...
Navigate to your free database instance by visiting the Neo4J Aura site, selecting 'Databases' from the menu, finding your free database and selecting 'Open with Neo4J Browser'.  After logging in with your credentials, you should be in the browser interface, which looks like Figure \@ref(fig:neo4j-interface).
...
Congratulations!  You've loaded the data into the graph database.  Now let's run a query to find the top 5 politicians who interacted with Christine Elliott based on weight (excluding Christine Elliott herself).
...
This should return a list of 5 with Robin Martin at the top with a weight of 872.  As another example, we can try to find out all interactions between LIB party politicians and IND party politicians.
...
`r if (knitr::is_latex_output()) '\\index{APOC procedures}'`This should return 4 interactions.  You can also try some procedures from Neo4J's APOC (Awesome Procedures on Cypher) library.  For example, try this to receive information on the distribution of the degrees of the nodes in the graph.
...
`r if (knitr::is_latex_output()) '\\index{Neo4J!working with in Python}'`The `r if (knitr::is_latex_output()) '\\index{packages!Python!py2neo@\\texttt{py2neo}}'``py2neo` package allows interaction with Neo4J servers from Python.  No preconfiguration is required.  Connection is established by means of a `Graph()` object.
...
4.  Using the online vertex and edge set for the `r if (knitr::is_latex_output()) '\\index{data sets!schoolfriends@\\texttt{schoolfriends}}'``schoolfriends` data set^[https://ona-book.org/data/schoolfriends_edgelist.csv and https://ona-book.org/data/schoolfriends_vertices.csv], create a new set of nodes in the Neo4J graph called `Student` nodes.  Give these nodes a `name`, `studentClass` and `studentGender` property from the data sets.
