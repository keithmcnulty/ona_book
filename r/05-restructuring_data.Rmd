...
`r if (knitr::is_latex_output()) '\\index{packages!Python!pandas@\\texttt{pandas}}'``r if (knitr::is_latex_output()) '\\index{packages!Python!numpy@\\texttt{numpy}}'`To illustrate similar approaches in Python, we will redo the work in Section \@ref(common-purchases).  First we will download the various datasets.
...
`r if (knitr::is_latex_output()) '\\index{scraping}'`First we will look at how to obtain a list of numbered scenes and the characters in each scene, through 'scraping' these details from the online script.  To help us with this we will use the `r if (knitr::is_latex_output()) '\\index{packages!R!rvest@\\texttt{rvest}}'``rvest` R package, which is designed for scraping information from web pages.
...
```{r chrome-console, echo = FALSE, fig.align = "center", fig.cap = "Viewing the script of Season 1 Episode 1 of Friends with the Elements console open in Google Chrome"}
...
This has generated a named character vector that contains a lot of different split out parts of the script, but most importantly it contains the lines from the script, for example:
...
Load the `r if (knitr::is_latex_output()) '\\index{data sets!parkreviews@\\texttt{park\\_reviews}}'``park_reviews` dataset from the `onadata` package or download it from the internet^[https://ona-book.org/data/park_reviews.csv].  This data contains the reviews of a collection of Yelp users on dog parks in the Phoenix, Arizona area.
...
6.  Use the methods in Section \@ref(doc-transform), including the `unique_pairs()` function, to create an edgelist for the character network for Episode 2 with the same edge weight based on the number of scenes the characters both spoke in.
...
9.  **Extension:**. Try to wrap the previous methods in a function that creates an edgelist for any arbitrary *Friends* episode found at `https://fangj.github.io/friends/`.  Add additional cleaning commands for unexpected formatting that you might see in different scripts.
