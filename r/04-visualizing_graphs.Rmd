...
The `draw()` function in `networkx` provides a basic visualization of a graph using `matplotlib` using a force-directed "spring" layout, as can be seen in \@ref(fig:py-karate-basic).  Remember also to set a seed to ensure reproducibility of the visualization.

```{python py-karate-basic, fig.align = "center", fig.cap = "Basic static visualization of Karate network", out.width = if (knitr::is_latex_output()) {"90%"}, dev = "png", dev.args = list(type = "cairo-png")}
import numpy as np
...
The `draw_networkx()` function has a much wider range of options for customizing the appearance of graphs.  For example, we can change the color of all or specific nodes or edges, or label specific nodes but not others, such as in Figure \@ref(fig:py-karate-additional).

```{python py-karate-additional, fig.align = "center", fig.cap = "Static visualization of Karate network with adjustments to color and labeling", out.width = if (knitr::is_latex_output()) {"90%"}, dev = "png", dev.args = list(type = "cairo-png")}
# set seed for reproducibility
np.random.seed(123)
...
A limited selection of layouts is available and can be applied to the static visualization.  For example, this is how to apply a circular layout, with the output in Figure \@ref(fig:py-karate-circ).

```{python py-karate-circ, fig.align = "center", fig.cap = "Static visualization of Karate network with circular layout", out.width = if (knitr::is_latex_output()) {"90%"}, dev = "png", dev.args = list(type = "cairo-png")}
# set seed for reproducibility
np.random.seed(123)
...
This is how to apply a Kamada-Kawai force-directed layout, with the output in Figure \@ref(fig:py-karate-shell).  Note that some layout algorithms like Kamada-Kawai make use of the `scipy` package and therefore this will need to be installed in your Python environment.

```{python py-karate-shell, fig.align = "center", fig.cap = "Static visualization of Karate network with Kamada-Kawai force-directed layout", out.width = if (knitr::is_latex_output()) {"90%"}, dev = "png", dev.args = list(type = "cairo-png")}
# set seed for reproducibility
np.random.seed(123)
