...
The `draw()` function in `networkx` provides a basic visualization of a graph using `matplotlib` using a force-directed "spring" layout, as can be seen in Figure \@ref(fig:pykaratebasic).  Remember also to set a seed to ensure reproducibility of the visualization.

```{python pykaratebasic, fig.align = "center", fig.cap = "Basic static visualization of Karate network", out.width = if (knitr::is_latex_output()) {"90%"}, dev = "png", dev.args = list(type = "cairo-png")}
import numpy as np
...
```{python pykarateadditional, fig.align = "center", fig.cap = "Static visualization of Karate network with adjustments to color and labeling", out.width = if (knitr::is_latex_output()) {"90%"}, dev = "png", dev.args = list(type = "cairo-png")}
# set seed for reproducibility
np.random.seed(123)
...
```{python pykaratecirc, fig.align = "center", fig.cap = "Static visualization of Karate network with circular layout", out.width = if (knitr::is_latex_output()) {"90%"}, dev = "png", dev.args = list(type = "cairo-png")}
# set seed for reproducibility
np.random.seed(123)
...
```{python pykarateshell, fig.align = "center", fig.cap = "Static visualization of Karate network with Kamada-Kawai force-directed layout", out.width = if (knitr::is_latex_output()) {"90%"}, dev = "png", dev.args = list(type = "cairo-png")}
# set seed for reproducibility
np.random.seed(123)
...
4.  Describe some likely challenges with large graph visualization which may make it harder to draw conclusions from them.
...
10. In you visualize the same graph twice using the same layout, in some cases the output may look different.  Why is this and what can be done to control it?
...
8.  Create a basic visualization of both graphs using a method of your choice.
9.  Experiment with different layouts for your visualization.  Which one do you prefer and why?
