...
To illustrate the `components()` function in `igraph` we will load up the `schoolfriends` edgelist dataset from an earlier chapter.  We will use reported friendships, create a directed graph and visualize it as in Figure \@ref(fig:viz-schoolfriends-rp).

```{r viz-schoolfriends-rp, fig.align = "center", fig.cap = "The directed graph of reported friendships from the `schoolfriends` data set"}
library(igraph)
library(ggraph)
library(dplyr)
...
```{r karate-py-conn, echo = FALSE, fig.align="center", fig.cap="Best Louvain partition of the `karate` graph", out.width = "90%"}
...
Let's reload our Facebook schoolfriends graph so that we can investigate communities and cliques inside it.
...
Six communities have been identified.  Let's compare the modularity of this community structure to the known communities of class and gender.
...
This community structure is certainly a better indicator of Facebook subgroups than gender or class.  We can visualize the class structure side by side with the Louvain community structure to try to interpret it, as in Figure \@ref(fig:schoolfriends-louvain).

```{r schoolfriends-louvain, fig.align = "center", fig.cap = "Optimal Louvain communities in the Facebook schoolfriends graph (left) versus class communities (right)"}
library(patchwork)
...
```
Visually, we can see that the Louvain community structure is a much better representation of dense friendship groups compared to the class structure, indicating that Facebook friendships tend to span across class structures.  We can also see that one of the Louvain communities contains the clique we identified earlier. 
...
ontariopol_edges <- read.csv("https://ona-book.org/data/ontariopol_edgelist.csv")
ontariopol_vertices <- read.csv("https://ona-book.org/data/ontariopol_vertices.csv") 
...
We see six commmunities.  Let's compare the modularity of this community structure to the political party structure.
...
```{r ontariopol-communities, fig.align = "center", fig.cap = "Twitter interaction of Ontario politicians segmented by their Louvain communities (left) and their political parties (right).  Louvain does a good job of identifying political alignment."}
# louvain communities
set.seed(123)
...
8. What do we mean when we describe the a community detection process as 'unsupervised'?   
9. What is meant be a clique in an undirected graph?  
10. What is a maximal clique and a largest clique in a graph?  Are maximal cliques always largest cliques?  Why or why not?

### Data exercises

For Exercises 1-3, load the `wikivote` edgelist from the `onadata` package or download it from the internet^[https://ona-book.org/data/wikivote.csv].  Recall that this data set represents votes from Wikipedia members to other Wikipedia members to nominate the `to` member to administrator status.  Create a directed graph from this data set.  Be careful with trying to visualize this graph as it is large.
...
For exercises 4-10, load the `email_edgelist` and `email_vertices` data sets from the `onadata` package or download them from the internet^[https://ona-book.org/daya/email_edgelist.csv and https://ona-book.org/data/email_vertices.csv].  This data set represents a network of emails sent between members of a large research institution.  The department of each member is included in the vertex data set.  Create an undirected graph from this data^[Note that the visualizations in these exercises might be demanding on low-memory/low-CPU machines. Proceed with caution.].
...
7. Visualize the graph color-coded by Louvain community, and visualize separately color-coded by ground truth department.  Compare the visualizations.  Can you describe any of the Louvain communities in terms of departments?
