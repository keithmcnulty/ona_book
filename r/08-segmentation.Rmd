# Components, Cliques and Communities {#community}
...
## Components, cliques and communities in R
...
For the next few examples we will return to *Zachary's Karate Club* network from Chapter \@ref(viz-graphs).  Let's load up and visualize that directed graph and mark the known leading actors Mr Hi and John A with larger vertices.
...
We see that the maximal cliques of size 5 that we identified are also the largest cliques in the `karate graph`, and both orient around Mr Hi.  The function `clique_num()` returns the size of the largest clique.

```{r}
clique_num(karate)
```

## Components, cliques and communities in Python