...
The early work in this chapter will use a graph which we will call $G_{14}$, and which is shown in Figure \@ref(fig:g14). This graph contains fourteen vertices labelled 1 thru 14, where all vertices are connected to at least one other vertex in the network.  This is known as a *connected graph*.  
...
Distances can be calculated using the `shortest_path_length()` function, either to produce all distances or to focus on a specific source and/or target.  This will return a dict if a single source or target is provided, or a tuple otherwise.
