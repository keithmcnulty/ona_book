...
`r if (knitr::is_latex_output()) '\\index{data sets!g14@\\texttt{g14}}'`The early work in this chapter will use a graph which we will call $G_{14}$, and which is shown in Figure \@ref(fig:g14). This graph contains fourteen vertices labelled 1 thru 14, where a path of edges exists between any pair of vertices.  This is known as a `r if (knitr::is_latex_output()) '\\index{graph!connected}'`*connected graph*.  
