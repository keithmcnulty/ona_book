...
\textbf{Thinking ahead:} If you already know how to load graphs in R or Python, you might want to take a look at a graph object now, and you will see how the object is structured and defined around the two set structure \(G = (V, E)\).  For example, try to create a graph from the data for the \textit{Bridges of K\"{o}nigsberg} problem using the \texttt{koenigsberg} edgelist in the \texttt{onadata} package or downloaded from the internet at \texttt{https://ona-data.org/data/koenigsberg.csv}.  Take a look at the vertex set or the edge set to see how they contain the structures discussed above. 
\end{minipage}
}
```
`r if (knitr::is_html_output()) '-->'`
...
Usually, trees are graphs where the edges represent some sort of hierarchical or nested relationship.  Figure \@ref(fig:boybands) shows a tree graph of my favorite boy bands, where an edge indicates that the vertex below is a member of the vertex above.  It seems like five is the magic number for a great boy band.
...
\colorbox{babyblueeyes}{
\begin{minipage}{\textwidth}
\textbf{Thinking ahead:} If you know how to, load up graph of romantic relationships in the TV Series \textit{Mad Men} using the \texttt{madmen\_edgelist} data set from the \textit{onadata} package or by downloading it from \texttt{https://ona-book.org/data/madmen\_edgelist.csv}.  Try to create a graph that contains the \texttt{Married} edge property and then try to query your graph to determine which relationships were marriage relationships.
\end{minipage}
}
```
`r if (knitr::is_html_output()) '-->'`
...
`r if (knitr::is_latex_output()) '<!--'`
:::thinkahead
**Thinking ahead:** If you still have the *Bridges of K&ouml;nigsberg* data loaded as a graph in R or Python from earlier, you should be able to generate its edgelist easily.  Try to find the right function to use to do this within your package of choice.
:::
`r if (knitr::is_latex_output()) '-->'`
...
\colorbox{babyblueeyes}{
\begin{minipage}{\textwidth}
\textbf{Thinking ahead:} If you still have the \textit{Bridges of K\"{o}nigsberg} data loaded as a graph in R or Python from earlier, you should be able to generate its edgelist easily.  Try to find the right function to use to do this within your package of choice.
\end{minipage}
}
```
`r if (knitr::is_html_output()) '-->'`
...
\colorbox{babyblueeyes}{
\begin{minipage}{\textwidth}
\textbf{Thinking ahead:} Again, with the \textit{Bridges of K\"{o}nigsberg} graph loaded in R or Python, you can turn it into an adjacency matrix easily.  Try to find the right function for this in your package of choice.  Note that in packages like \texttt{networkx} in Python, the format is slightly different and is called an \textit{adjacency list}.
\end{minipage}
}
```
`r if (knitr::is_html_output()) '-->'`
...
We see the expected directed multigraph with 3 vertices and 16 edges.  If we wish to use create the weighted graph in Figure \@ref(fig:weightedflights), we add `weighted = TRUE` to the arguments.
...
As we noted in Section \@ref(rep-graphs), edgelists are usually sufficient to descibe a graph when isolates are not of concern.  The function `graph_from_edgelist()` works fine for this purpose, but is lacking in flexibility when graphs contain isolates or other properties that you would ideally like to load on creation.  However, the function `graph_from_data_frame()` allows you to create a more flexible graph directly from dataframes containing the required data.
...
\colorbox{babyblueeyes}{
\begin{minipage}{\textwidth}
\textbf{Playing around:} The functions in this section are not the only functions in the \texttt{igraph} package that build graphs from data, but they are by far the most commonly used ones.  By typing \texttt{?graph\_from} in your R console and looking at the functions that autocomplete, you can see some of the other functions that build graphs from data.  Try playing around with them if you are curious.
\end{minipage}
}
```
`r if (knitr::is_html_output()) '-->'`
