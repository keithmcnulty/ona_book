...
## Character interaction in Victor Hugo's novel *Les Miserables*

Load the `r if (knitr::is_latex_output()) '\\index{data sets!lesmis@\\texttt{lesmis}}'``lesmis` data set from the `onadata` package or download it from the internet^[https://ona-book.org/data/lesmis.csv].  This edgelist represents the network of characters in the novel *Les Miserables* by Victor Hugo^[This data set is taken from @lesmispaper.].  An edge exists between two characters if they interact with each other in the book, and the `weight` column indicates the number of interactions.
