...
Code can be written and executed and the results displayed inline using backticks.  For example, recalling our `chinook_employees` dataset from earlier and writing 
...
inline will display `r nrow(chinook_employees)` in the final document^[This assumes the dataset has been loaded afresh without removing any data from it].  Entire code blocks can be included and executed by using triple-backticks.  The following code block:
...
5.  `r if (knitr::is_latex_output()) '\\index{data sets!ugtests@\\texttt{chinook_customers}}'`Load the `chinook_customers` data set via the `onadata` package or download it from the internet^[https://ona-book.org/data/chinook_customers.csv].  Calculate the dimensions of `chinook_customers` and view the first three rows only. 
...
11.  Create a scatter plot using the built-in `mtcars` dataset with data from the column named `mpg` plotted on the $y$ axis and data from the column named `hp` plotted on the $x$ axis.
12.  Using the same `mtcars` dataset, convert the data in the `cyl` column to a factor with three levels.  Plot a histogram of the count of observations at each of the three `cyl` levels.
