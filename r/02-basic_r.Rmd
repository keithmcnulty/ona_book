...
`r if (knitr::is_latex_output()) '\\index{R!loading data}'`To work with data in R, you usually need to pull it in from an outside source into a dataframe^[R also has some built-in data sets for testing and playing with.  For example, check out `mtcars` by typing it into the terminal, or type `data()` to see a full list of built-in data sets.].  R facilitates numerous ways of importing data from simple `.csv` files, from Excel files, from online sources or from databases.  Let's load a data set that we will use later---the `workfrance_edgelist` data set, which contains network information from an experiment in a French workplace conducted in 2015.  The `read.csv()` function can accept a URL address of the file if it is online.

```{r}
# url of data set 
url <- "http://ona-book.org/data/workfrance_edgelist.csv"
# load the data set and store it as a dataframe called workfrance_edges
workfrance_edges <- read.csv(url)
```
...
```{r}
dim(workfrance_edges)
```

```{r}
# hundreds of rows, so view first few
head(workfrance_edges)
```
...
```{r}
summary(workfrance_edges)
```

`r if (knitr::is_latex_output()) '\\index{R!missing data}'`Note that there is no missing data in this dataframe.  Missing data is identified by a special `NA` value in R.  This should not be confused with `"NA"`, which is simply a character string.  

Let's look at another data set: the `airquality` dataset, which is a record of some air quality measures taken in New York in 1973.  This is an in-built dataframe in R and therefore it simply exists in your R session without needing to load it.   The function `is.na()` will look at all values in a vector or dataframe and return `TRUE` or `FALSE` based on whether they are `NA` or not.  By adding these up using the `sum()` function, it will take `TRUE` as 1 and `FALSE` as 0, which effectively provides a count of missing data.
...
If we need to change the type of a column in a dataframe, we can use the `as.numeric()`, `as.character()`, `as.logical()` or `as.factor()` functions.  For example, given that there are only five unique values for the `Month` column in `airquality`, we may want to convert it from its current integer form to a factor.
...
```{r}
airquality_month5 <- subset(airquality, subset = Month == 5)
head(airquality)
```
...
```{r}
(high_ozone <- subset(airquality, subset = Ozone >= 120))
```

To select specific columns use the `select` argument.

```{r}
airquality_ozone_wind <- subset(airquality, 
                                 select = c("Ozone", "Wind"))
head(airquality_ozone_wind)
```
...
```{r}
low_ozone <- subset(airquality, subset = Ozone < 6)
# bind the rows of low_sales and high_sales together
low_and_high_ozone = rbind(low_ozone, high_ozone)
low_and_high_ozone
```
...
airquality_ozone_wind <- subset(airquality, 
                     select = c("Ozone", "Wind"))
airquality_temp_day <- subset(airquality, 
                        select = c("Temp", "Day"))
# bind the columns to create a dataframe with four columns
full_df <- cbind(airquality_ozone_wind, airquality_temp_day)
head(full_df)
```
...
We can test our function by using `airquality` data set.

```{r}
df_report(airquality)
```
...
```{r, eval = FALSE}
subset(airquality, subset = Temp = 78)
```
...
This helps you see that you have used `Temp = 720` as a condition to subset your data, when you should have used `Temp == 720` for precise equality.
...
`r if (knitr::is_latex_output()) '\\index{R!scatter plots}'`The simplest plot function in base R is `plot()`.  This performs basic X-Y plotting.  As an example, this code will generate a scatter plot of `Ozone` against `Temp` in the `airquality` data set, with the results displayed in Figure \@ref(fig:basicscatter).  Note the use of the arguments `main`, `xlab` and `ylab` for customizing the axis labels and title for the plot.
...
`r if (knitr::is_latex_output()) '\\index{R!packages!ggplot2@\\texttt{ggplot2}}'`By far the most commonly used specialist plotting and graphing package in R is `ggplot2`.  `ggplot2` allows the flexible construction of a very wide range of charts and graphs, but uses a very specific command grammar which can take some getting used to.  However, once learned, `ggplot2` can be an extremely powerful tool.  Later in this book we will make a lot of references to `ggplot2` and some of its extension packages like `ggmap` and `ggraph`.  A great learning resource for `ggplot2` is @ggplot2.  Here are some examples of how to recreate the plots from the previous section in `ggplot2` using its layered graphics grammar.
...
Code can be written and executed and the results displayed inline using backticks.  For example, recalling out `workfrance_edges` dataset from earlier and writing 
...
````
`r`r ''` nrow(workfrance_edges)`
````

inline will display `r nrow(workfrance_edges)` in the final document.  Entire code blocks can be included and executed by using triple-backticks.  The following code block:

````
```{r}`r ''`
# show the first few rows of workfrance_edges
head(workfrance_edges)
```
````
